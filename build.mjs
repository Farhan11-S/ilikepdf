...
#
# Every block is wrapped so a host missing the module serves the file
# uncompressed instead of returning a 500. Note that <IfModule> only asks
# whether a module is loaded — it does nothing about AllowOverride, which is a
# separate way for the same directive to 500 the whole site.
#
# So everything below is deliberately one AllowOverride class: RewriteRule,
# Header and AddType all need FileInfo. There is no \`Options -Indexes\` here on
# purpose — it needs AllowOverride Options, which shared hosts commonly
# withhold, and wrapping it in <IfModule mod_autoindex.c> would not have helped.
# vendor/index.html stops the listing instead. \`DirectoryIndex index.html\` is
# gone for the same reason: it needs AllowOverride Indexes, and index.html is
# mod_dir's built-in default anyway.
...
<IfModule mod_rewrite.c>
  ...
  RewriteRule \\.html\\.(br|gz)$ - [T=text/html,E=enc:1]
  RewriteRule \\.js\\.(br|gz)$   - [T=application/javascript,E=enc:1]
  RewriteRule \\.svg\\.(br|gz)$  - [T=image/svg+xml,E=enc:1]
</IfModule>
